...
def read_stories_page():
    ...
    try:
        ...
        if not stories:
            st.info(
                "📚 No stories have been shared yet. Be the first to share your story!"
            )
            return
        ...
        with col1:
            ...
            all_categories = list(
                set(
                    [
                        story.category.replace("_", " ").title()
        ...
        if selected_category != "All Categories":
            filtered_stories = [
                ...
                for s in stories
                if s.category
                and s.category.replace("_", " ").title() == selected_category
            ]
        ...
        for i, story in enumerate(filtered_stories):
            with st.container():
                ...
                with col1:
                    # Display cover photo
                    if story.thumbnail_image_path and os.path.exists(
                        story.thumbnail_image_path
                    ):
                        try:
                            image = Image.open(story.thumbnail_image_path)
                            st.image(
                                image, caption="Story Cover", use_column_width=True
                            )
                ...
                with col2:
                    ...
                    # Story preview
                    preview_text = (
                        story.transcript[:150] + "..."
                        if len(story.transcript) > 150
                        else story.transcript
                    )
                    ...
                    if st.button(
                        f"📖 Read Full Story", key=f"read_{story.id}", type="secondary"
                    ):
